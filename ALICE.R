...
ALICE_pipeline<-function(DTlist,folder="",cores=8,iter=50,nrec=5e5,P_thres=0.001,Read_thres=1,cor_method="BH")
{
  make_rda_folder(DTlist,folder,Read_thres=Read_thres) #generate .rda files for CDR3aa gen prob estimation for each VJ
  ...
  results<-parse_rda_folder(DTlist,folder,volume = cores*iter*nrec/3,Read_thres=Read_thres) #parse res_ files
